...
class GeoLocatorPlugin
	...
	*/
	static function _get_optimized_file (
	...
	*/
	static function content_filter ( $the_content )
		...
		if ( $options['show_map'] )
			{
			$the_new_content .= self::_get_optimized_file ( dirname ( __FILE__ ).'/index.html', '/*##START_JAVASCRIPT_MAP##*/', '/*##END_JAVASCRIPT_MAP##*/' );
			}
		else
			{
		...
		if ( $options['show_debug'] )
			{
			$the_new_content .= self::_get_optimized_file ( dirname ( __FILE__ ).'/index.html', '/*##START_JAVASCRIPT_DEBUG##*/', '/*##END_JAVASCRIPT_DEBUG##*/' );
			}
		else
			{
	...
	*/
	static function printAdminPage ( )
		...
		echo $wrapper_head.$report.self::_get_optimized_file ( dirname ( __FILE__ ).'/geolocator_admin_form.html' ).$wrapper_tail.'</div>';
		}
	...
	*/
	static function _getAdminOptions ( )
		...
		if ( !$GeoLocatorOptions['show_long_lat'] )
			{
